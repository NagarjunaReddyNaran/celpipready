...
export function OnboardingForm({ initialName }: { initialName: string }) {
  ...
  async function handleSkip() {
    setSaving(true);
    try {
      await save({ ...form, goalType: form.goalType || "other" });
    } catch {
      // Skip silently — user lands on dashboard regardless
    }
    router.push("/dashboard");
  }
            ...
            <div className="space-y-2 mb-6">
              {GOALS.map(g => (
                ...
                >
                  ...
                  {form.goalType === g.value && <span className="ml-auto text-blue-600 text-lg">✓</span>}
                </button>
              ))}
            </div>
            <div className="flex gap-3">
              <button onClick={() => setStep(1)} className="flex-1 border border-slate-300 py-3 rounded-xl text-sm font-medium hover:bg-slate-50">
                ← Back
              </button>
              <button
                onClick={() => setStep(3)}
              ...
              <div className="grid grid-cols-2 gap-2">
                {SECTIONS.map(s => (
                  ...
                  >
                    ...
                    {form.weakAreas.includes(s.value) && <span className="ml-auto text-blue-600 text-sm">✓</span>}
                  </button>
                ))}
              </div>
            </div>
